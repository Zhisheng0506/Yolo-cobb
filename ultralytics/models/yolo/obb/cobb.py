...
class COBBCoder:
    """PyTorch implementation of the Continuous OBB (COBB) coder."""
    ...
    def __init__(self, ratio_type: str = "sig", pow_iou: float = 1.0, eps: float = 1e-6):
        ...
        if self.cfg.ratio_type != "sig":  # TODO: add 'ln' support
    ...
    def rotated_to_hbbox(self, rbboxes: torch.Tensor) -> torch.Tensor:
        polys = self.rbox_to_poly(rbboxes)
        min_xy = polys.min(dim=-2).values
        max_xy = polys.max(dim=-2).values
    ...
    def encode(self, rbboxes: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if rbboxes.numel() == 0:
            return (
                torch.zeros((0, 1), device=rbboxes.device, dtype=rbboxes.dtype),
                torch.zeros((0, 4), device=rbboxes.device, dtype=rbboxes.dtype),
            )
        polys = self.rbox_to_poly(rbboxes)
        ...
        score_targets = []
        for i in range(4):
            score_targets.append(batch_probiou(candidates[:, i], rbboxes))
        score_targets = torch.stack(score_targets, dim=-1).clamp_(0)
